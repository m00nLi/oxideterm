...
mod tests {
    ...
    fn rect(x: f32, y: f32, width: f32, height: f32) -> LogicalWindowRect {
        LogicalWindowRect {
            x,
            y,
            width,
    ...
    #[test]
    fn restore_keeps_a_window_on_its_visible_secondary_display() {
        let primary = rect(0.0, 0.0, 1920.0, 1040.0);
        let secondary = rect(-1440.0, 0.0, 1440.0, 860.0);
        ...
        assert_eq!(
            restore_window_rect(Some(saved), &[primary, secondary], primary),
            rect(-1320.0, 40.0, 1100.0, 760.0)
        );
    }

    #[test]
    fn restore_recenters_an_offscreen_window_on_the_primary_display() {
        let primary = rect(0.0, 0.0, 1920.0, 1040.0);
        let saved = WindowGeometry {
            x: 6000,
            y: 4000,
        ...
        assert_eq!(
            restore_window_rect(Some(saved), &[primary], primary),
            rect(460.0, 170.0, 1000.0, 700.0)
        );
    }

    #[test]
    fn first_launch_scales_to_the_visible_display_and_respects_minimum_size() {
        let compact_display = rect(0.0, 0.0, 1024.0, 700.0);

        assert_eq!(
            restore_window_rect(None, &[compact_display], compact_display),
            rect(102.5, 50.0, 819.0, 600.0)
        );
    }
}
