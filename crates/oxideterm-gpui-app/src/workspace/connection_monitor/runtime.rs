...
mod tests {
    ...
    #[test]
    fn pool_usage_is_bounded_and_handles_zero_capacity() {
        assert_eq!(connection_runtime_pool_usage(4, 8), 0.5);
        assert_eq!(connection_runtime_pool_usage(4, 0), 0.0);
        assert_eq!(connection_runtime_pool_usage(12, 8), 1.0);
    }

    #[test]
    fn attention_projection_only_includes_actionable_states() {
        assert!(connection_runtime_state_needs_attention(
            &ConnectionPoolEntryState::Reconnecting
        ));
        assert!(connection_runtime_state_needs_attention(
            &ConnectionPoolEntryState::LinkDown
        ...
        assert!(connection_runtime_state_needs_attention(
            &ConnectionPoolEntryState::Error("failed".to_string())
        ));
        assert!(!connection_runtime_state_needs_attention(
            &ConnectionPoolEntryState::Active
        ));
        assert!(!connection_runtime_state_needs_attention(
            &ConnectionPoolEntryState::Idle
        ));
    }

    #[test]
    fn runtime_overview_copy_exists_in_every_locale() {
        let locales = [
            Locale::De,
            Locale::En,
            Locale::EsEs,
        ...
        let keys = [
            "connections.monitor.overview_pool_usage",
            "connections.monitor.overview_consumers",
            "connections.monitor.overview_attention",
            "connections.monitor.overview_healthy",
            "connections.monitor.overview_error",
            "connections.monitor.overview_refresh_interval",
        ];
        let english = I18n::new(Locale::En);

        // Operational labels must not silently fall back to English in any
        // shipped locale because they remain visible during failures.
        for locale in locales {
            let i18n = I18n::new(locale);
            for key in keys {
                let translated = i18n.t(key);
                assert_ne!(translated, key, "{locale:?} missing {key}");
                if locale != Locale::En && key != "connections.monitor.overview_error" {
                    assert_ne!(
                        translated,
                        english.t(key),
