...
impl WorkspaceApp {
    ...
    /// Modal-owner key route. The blocking key capture in the root render
    /// chain handles printable editing keys first; this entry point mirrors
    /// the portal routing used by the other main-window modals.
    pub(in crate::workspace) fn handle_tab_rename_dialog_key(
        ...
        self.handle_tab_rename_key(event, cx)
    }
}
