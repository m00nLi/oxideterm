...
const DOCKER_SAMPLE_COMMAND_UNIX: &str = concat!(
    ...
    "if printf '%s\\n' \"$oxide_docker_ps\" | head -n 1 | grep -q '^{'; then ",
    ...
    "fi; ",
    "else ",
    "oxide_docker_ps_table=$(docker ps -a --no-trunc 2>/dev/null); ",
    "oxide_docker_table_status=$?; ",
    "if [ \"$oxide_docker_table_status\" -eq 0 ]; then ",
    "printf '%s\\n' \"$oxide_docker_ps_table\" | awk -F '  +' 'NR > 1 && NF >= 6 { status=$5; state=$5; sub(/ .*/, \"\", state); ports=(NF >= 7 ? $6 : \"\"); printf \"PS\\t%s\\t%s\\t%s\\t%s\\t%s\\t%s\\n\", $1, $NF, $2, state, status, ports }'; ",
    "else ",
    "printf '__OXIDE_DOCKER_ERROR__\\t%s\\n' \"$(printf '%s' \"$oxide_docker_ps_table\" | head -n 1 | tr '\\t' ' ')\"; ",
    "fi; ",
    "fi; ",
    "else ",
