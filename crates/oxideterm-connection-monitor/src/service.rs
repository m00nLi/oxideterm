...
const SERVICE_SAMPLE_COMMAND_LINUX_SYSTEMD: &str = concat!(
    ...
    "oxide_service_bare_ids=$(printf '%s\\n' \"$oxide_service_ids\" | sed 's/\\.service$//'); ",
    "oxide_service_details=$(systemctl show --no-pager --property=Id,LoadState,ActiveState,SubState,UnitFileState,MainPID,Description $oxide_service_bare_ids 2>&1); ",
    "oxide_service_detail_status=$?; ",
    "if [ \"$oxide_service_detail_status\" -ne 0 ] && ! printf '%s\\n' \"$oxide_service_details\" | grep -q '^Id='; then ",
    "printf '%s\\n' \"$oxide_service_units\" | awk 'BEGIN{OFS=\"\\t\"} NF >= 5 { description=\"\"; for (i=5; i<=NF; i++) description = description (i>5 ? \" \" : \"\") $i; print \"ROW\", $1, $2, $3, $4, \"\", \"\", description }'; ",
    "else ",
