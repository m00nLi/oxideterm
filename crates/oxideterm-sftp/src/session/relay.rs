...
impl SftpSession {
    ...
    pub async fn relay_dir_to(
        ...
        let requested_parallelism = transfer_manager
            ...
            .unwrap_or(1);
        // Either endpoint being single-channel forces the relay batch into one
        // lane; a sibling channel would kill that transport.
        let requested_parallelism = if self.single_channel || destination.single_channel {
            1
        } else {
